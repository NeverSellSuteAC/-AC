...
const util = {
  /**
   * 根据一个key的值,从列表中获取该key对应对象的其他key的值
   * @param {*} list    列表
   * @param {*} value   值
   * @param {*} key1    值的key
   * @param {*} key2    要获取的其他的值的key
   */
  getStr(list, value, key1, key2) {
    if (list == null) {
      list = [];
    }
    ...
    let option =
      formItems.forEach(item => {
        ...
        if (item instanceof String) {
          ...
          if (valueList != null) {
            filterItem.value = valueList[0];
            filterItem.endValue = valueList[1];
          }
          //如果是时间类型,切两个时间相同
          if (item.attrType == "java.util.Date" && filterItem.value == filterItem.endValue) {
            let date = new Date(filterItem.endValue);
            date.setDate(date.getDate() + 1);
            filterItem.endValue = this.formatDate(date);
          } else if (item.attrType == "java.util.Date") {
            filterItem.endValue = this.formatDate(new Date(filterItem.endValue))
          }
        }
        else {
          filterItem = Object.assign({

            ...
            <table class="el-table">
              ...
              <tr v-bind:key="index" v-for="(logger, index) in loggers">
                ...
                <td>
                  <el-button-group>
                    <el-button
                      :type="(logger.level=='TRACE') ? 'primary' : ''"
                      @click="changeLevel(logger.name, 'TRACE')"
                      size="mini"
                    >TRACE
                    </el-button>
                    <el-button
                      :type="(logger.level=='DEBUG') ? 'success' : ''"
                      @click="changeLevel(logger.name, 'DEBUG')"
                      size="mini"
                    >DEBUG
                    </el-button>
                    <el-button
                      :type="(logger.level=='INFO') ? 'info' : ''"
                      @click="changeLevel(logger.name, 'INFO')"
                      size="mini"
                    >INFO
                    </el-button>
                    <el-button
                      :type="(logger.level=='WARN') ? 'warning' : ''"
                      @click="changeLevel(logger.name, 'WARN')"
                      size="mini"
                    >WARN
                    </el-button>
                    <el-button
                      :type="(logger.level=='ERROR') ? 'danger' : ''"
...
<script>
    ...
    export default {
        ...
        watch: {
            filter(val) {
                let array = [];
                for (const key in this.allloggers) {
                    let item = this.allloggers[key];
